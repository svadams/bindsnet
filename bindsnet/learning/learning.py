...
class WeightDependentPostPre(LearningRule):
    ...
    def _connection_update(self, **kwargs) -> None:
        ...
        if self.nu[0]:
            ...
            tmp_update = self.nu[0] * outer_product * (self.connection.w - self.wmin)
            update -= tmp_update
            print('LTD', tmp_update)

        # Post-synaptic update.
        if self.nu[1]:
            outer_product = self.reduction(torch.bmm(source_x, target_s), dim=0)
            tmp_update = self.nu[1] * outer_product * (self.wmax - self.connection.w)
            update += tmp_update
            print('LTP', tmp_update)
